...
class Practicum {
    ...
    private static void printFlightInformation(
        ...
        ZonedDateTime departure = ZonedDateTime.ofLocal(,departureAirport.getZone());

        // Выведите информацию о том, между какими городами будет перелёт.
        System.out.println("Ваш билет на рейс " + departureAirport + " - " + arrivalAirport + ": ");

        // Найдите продолжительность полёта.
        Duration flightDuration = Duration.between(departure,departure.plusHours(flightDurationHours));
        // Найдите время прибытия с учётом зоны прилёта.
        ZonedDateTime arrival = ZonedDateTime.ofLocal(,arrivalAirport.getZone());
        ...
        String formattedArrivalTime = departure.format(formattedDepartureTime);
        // Только время вылета
        String departureTimeOnly = departure.format(formattedDepartureTime);
